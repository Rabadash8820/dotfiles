...
To use in VS Code devcontainers, update the following VS Code User settings:
...
You can do some hacks on the command line to make this work
(setting `GNUPGHOME` to the other operating system's `.gnpug` directory, symlinking the directory, etc.)
but then graphical programs like GitKraken or VS Code will have trouble locking the keyring to list/edit keys.
...
Basically, create keys with `gpg --full-generate-key`, and follow the prompts to create a Ed25519 key pair,
then edit that key to create a signing-only subkey (with the same passphrase). Store key passphrases in a password manager like 1Password.
...
# Copy the whole output to GitHub, including the `PGP PUBLIC KEY BLOCK` lines
```
...
- In VS Code settings, check "Git: Enable Commit Signing" (and similar checkboxes for Git extensions).
...
When using different configs (e.g., user email/signingkey) for different repos,
consider using Git's [conditional includes](https://git-scm.com/docs/git-config#_conditional_includes).
For example, you could define default configs in your global git config (`~/.gitconfig`),
then define overrides for working in repos from a specific GitHub Organization by creating a `~/.gitconfig-<org>` file (formatted like a normal gitconfig file),
and conditionally include it in your global git config like so:
...
Verify that the setup is working by running `git config get <config>` in different repos.
